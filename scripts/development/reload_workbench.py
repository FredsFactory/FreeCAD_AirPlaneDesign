...
def reload_workbench():
    ...
    # Importer FreeCAD seulement quand nécessaire
    try:
        import FreeCAD as App
        import FreeCADGui as Gui
    except ImportError:
        print("⚠️ FreeCAD non disponible - rechargement en mode test")
        return False
    ...
    try:
        ...
        if (
            hasattr(current_wb, "__class__")
            and "AirPlaneDesign" in current_wb.__class__.__name__
        ):
            # Si on est déjà dans le workbench, on le désactive puis réactive
            try:
                # Essayer d'abord le workbench par défaut de FreeCAD
                Gui.activateWorkbench("PartWorkbench")
            except:
                try:
                    # Si PartWorkbench n'existe pas, essayer autre chose
                    Gui.activateWorkbench("NoneWorkbench")
                except:
                    # En dernier recours, ne pas changer de workbench
                    print("⚠️ Impossible de changer de workbench temporairement")

            # Réactiver notre workbench
