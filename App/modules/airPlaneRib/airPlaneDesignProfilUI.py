...
class SelectObjectUI:
    def __init__(self):
        path_to_ui = (
            ...
            + "Mod/AirPlaneDesign/resources/selectRibProfil.ui"
        )
    ...
    def setupUi(self):
        ...
        self.form.NACANumber.editingFinished.connect(
            self.form.listProfil.clearSelection
        )
    ...
    def updateRibNACA(self):
        ...
        try:
            coords = generateNacaCoords(
                number,
                self.form.nacaNbrPoint.value(),
                self.form.finite_TE.isChecked(),
                True,
                self.form.chord.value(),
                0,
                0,
                0,
                0,
        ...
        for v in coords:
            ...
            self.form.profilTable.setItem(
                row_number, 0, QtGui.QTableWidgetItem(str(v.x))
            )
            self.form.profilTable.setItem(
                row_number, 1, QtGui.QTableWidgetItem(str(v.z))
            )
