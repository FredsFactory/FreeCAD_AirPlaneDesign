...
from App.xfoil.test_airfoils import naca0012

import FreeCAD.Plot as Plot


FreeCADGui.addLanguagePath(":/translations")
# Chemin vers les icônes dans le répertoire racine du workbench (remonte 4 niveaux: airPlaneRib → modules → App → AirPlaneDesign)
...
class WingRib:
    ...
    def __init__(
        ...
        obj.addProperty(
            ...
            "Coordinates",
            "Rib",
            "Vector list that defines the airfoil's geometry",
        ).Coordinates = []

        obj.Placement = FreeCAD.Placement(
            FreeCAD.Vector(_x, _y, _z),
            FreeCAD.Rotation(FreeCAD.Vector(_xrot, _yrot, _zrot), _rot),
    ...
    def onChanged(self, fp, prop):
        ...
        if (str(prop) == "RibProfil") and (
            fp.PropertiesList.__contains__("Coordinates")
        ):
            print("Rib onChanged:")
            fp.Coordinates = []
        if str(prop) == "wingkey":
    ...
    def execute(self, fp):
        ...
        if fp.NacaProfil == "":
            face, fp.Coordinates = process(
                fp.RibProfil,
                fp.Chord,
                fp.Placement.Base.x,
                fp.Placement.Base.y,
                fp.Placement.Base.z,
                fp.Placement.Rotation.Axis.x,
                fp.Placement.Rotation.Axis.y,
                fp.Placement.Rotation.Axis.z,
            ...
            face, fp.Coordinates = generateNaca(
                fp.NacaProfil,
                fp.NacaNbrPoint,
                fp.finite_TE,
                True,
...
class RibTaskPanel:
    ...
    def __init__(self, vobj):
        self.obj = vobj
        path_to_ui = (
            FreeCAD.getUserAppDataDir() + "Mod/AirPlaneDesign/resources/ribTaskPanel.ui"
        )
    ...
    def updateGraphicsViewRib(self):
        ...
        for row_number, row_data in enumerate(coords):
            ...
            for col_number, data in enumerate(row_data):
                self.form.profilTable.setItem(
                    row_number, col_number, QtGui.QTableWidgetItem(str(data))
                )
...
class CommandWingRib:
    ...
    def GetResources(self):
        ...
        return {
            "Pixmap": iconpath,
            "MenuText": QtCore.QT_TRANSLATE_NOOP("Create_a_Rib", "Create a Rib"),
        }
        ...
        if r:
            if editor.form.NACANumber.text() == "":
                ...
                WingRib(
                    ...
                    0,
                    0,
                    editor.form.thickness.value(),
                    editor.form.useSpline.isChecked(),
                    _splitSpline=editor.form.splitSpline.isChecked(),
                )
                ...
                WingRib(
                    ...
                    b.text(),
                    True,
                    int(editor.form.nacaNbrPoint.value()),
                    editor.form.chord.value(),
                    0,
                    0,
                    0,
                    ...
                    editor.form.thickness.value(),
                    editor.form.useSpline.isChecked(),
                    editor.form.finite_TE.isChecked(),
                    editor.form.splitSpline.isChecked(),
                )
                ViewProviderWingRib(a.ViewObject)
            FreeCAD.ActiveDocument.recompute()
        else:
