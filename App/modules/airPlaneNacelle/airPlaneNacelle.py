...
# Chemin vers les ressources dans le répertoire racine du workbench (remonte 4 niveaux: airPlaneNacelle → modules → App → AirPlaneDesign)
...
apWB_ui_file = os.path.join(apWB_resources_path, "nacelleTaskPanel.ui")
FreeCADGui.addLanguagePath(":/translations")


class Nacelle:
    def __init__(
        self, obj, _length, _diameter, _nType, _XMaxRel, _nbPoints, _useSpline=True
    ):
        """Nacelle properties"""
        obj.Proxy = self
        obj.addProperty(
            ...
            "nacelleLength",
            "nacelle",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Length"),
        ).nacelleLength = _length
        obj.addProperty(
            "App::PropertyLength",
            "nacelleDiameter",
            "nacelle",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Diameter"),
        ).nacelleDiameter = _diameter
        obj.addProperty(
            "App::PropertyFloat",
            "nacelleXMaxRel",
            "nacelle",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Diameter"),
        ).nacelleXMaxRel = _XMaxRel
        obj.addProperty(
            "App::PropertyString",
            "nacelleType",
            "nacelle",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Type"),
        ).nacelleType = _nType
        obj.addProperty(
            "App::PropertyBool",
            "useSpline",
        ...
        obj.addProperty(
            ...
            "nbPoints",
            "nacelle",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Number of Points"),
        ).nbPoints = _nbPoints
    ...
    def execute(self, fp):
        ...
        if fp.nacelleType == "Lyon":
            coords = libAeroShapes.getLyonCoords(
                fp.nacelleLength, fp.nacelleDiameter, fp.nbPoints
            )
        else:
            if fp.nacelleType == "EllipseCos":
                coords = libAeroShapes.getHoernerCoords(
                    fp.nacelleLength, fp.nacelleDiameter, fp.nacelleXMaxRel, fp.nbPoints
                )
            else:
                if fp.nacelleType == "Duhamel":
                    coords = libAeroShapes.getDuhamelCoords(
                        fp.nacelleLength, fp.nacelleDiameter, fp.nbPoints
                    )
                else:  #  fp.nacelleType = "NACA"
                    coords = libAeroShapes.getNACACoords(
                        fp.nacelleLength, fp.nacelleDiameter, fp.nbPoints
                    )
...
class ViewProviderskNacelle:
    def __init__(self, obj):
        ...
        obj.addProperty(
            "App::PropertyColor", "Color", "nacelle", "Color of the nacelle sketch"
        ).Color = (1.0, 0.0, 0.0)
        obj.Proxy = self
...
class CommandNacelle:
    ...
    def GetResources(self):
        return {
            "Pixmap": apWB_icon_png,
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Create_a_Nacelle", "Create a nacelle"
            ),
        }
    ...
    def Activated(self):
        ...
        Nacelle(
            sk,
            self.form.sbLength.value(),
            self.form.sbDiameter.value(),
            nType,
            XMaxRel,
            self.form.sbNbPoints.value(),
            self.form.cbSpline.isChecked(),
        )
