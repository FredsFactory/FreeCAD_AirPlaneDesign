...
# Chemin vers les icônes dans le répertoire racine du workbench (remonte 4 niveaux: airPlaneWPanel → modules → App → AirPlaneDesign)
smWB_icons_path = os.path.join(
        ...
        if len(r.Shape.OuterWire) > 0:
            ...
            try:
                ...
                FreeCAD.Console.PrintError(
                    "Cannot make a wire. Creation of surface is not possible !\n"
                )
                return
...
class WingPanel:
    def __init__(
        ...
        obj.addProperty(
            ...
            "Base",
            "WingPanel",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Tip Rib of the panel"),
        ).Base = _rootRib  #
        obj.addProperty(
            "App::PropertyLink",
            "Envelope",
            "WPRibs",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Eveloppe of wing"),
        ).Envelope = _envelope
        obj.addProperty(
            "App::PropertyLink",
            "Path",
            "WPRibs",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Path of wing"),
        ).Path = _path
        ...
        obj.addProperty(
            ...
            "Ribs",
            "WPRibs",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Ribs of the panel"),
        ).Ribs = _ribs
        ...
        obj.addProperty(
            ...
            "TipChord",
            "WPRibs",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Tip Chord"),
        ).TipChord = _tipChord
        obj.addProperty(
            "App::PropertyLength",
            "RootChord",
            "WPRibs",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Root Chord"),
        ).RootChord = _rootChord
        obj.addProperty(
            "App::PropertyLength",
            "PanelLength",
            "Design",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Panel Length"),
        ).PanelLength = _panelLength

        # need to convert into function not attributes
        obj.addProperty(
            "App::PropertyVector", "Axis", "WingPanel", "Direction axis"
        ).Axis = axis
        obj.addProperty(
            "App::PropertyLinkList", "Hullcurves", "WingEdge", "Bounding curves"
        ).Hullcurves = [_leadingedge, _traillingedge]
        ...
        obj.addProperty(
            ...
            QtCore.QT_TRANSLATE_NOOP(
                "App::Property", "Select the leading edge of the panal, line or Spline"
            ),
        ).Edges = []
        obj.addProperty(
            "App::PropertyLink",
        ...
        obj.addProperty(
            ...
            "WingEdges",
            QtCore.QT_TRANSLATE_NOOP(
                "App::Property", "Select the trailing edge of the panel, line or Spline"
            ),
        ...
        obj.addProperty(
            ...
            "TipTwist",
            "Design",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Tip Twist"),
        ).TipTwist = _tipTwist
        obj.addProperty(
            "App::PropertyAngle",
            "Dihedral",
            "Design",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Dihedral"),
        ).Dihedral = _dihedral
        ...
        obj.addProperty(
            ...
            "Solid",
            "Design",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Solid"),
        ).Solid = False
        obj.addProperty(
            "App::PropertyBool",
            "Surface",
            "Design",
            QtCore.QT_TRANSLATE_NOOP("App::Property", "Surface"),
        ).Surface = False
        obj.addProperty(
            "App::PropertyBool",
            "Structure",
        ...
        obj.addProperty(
            "App::PropertyQuantity", "Items", "Design", "Nr. of array items"
        ).Items = items
        obj.addProperty(
            "App::PropertyFloat",
            "OffsetStart",
        ...
        obj.addProperty(
            "App::PropertyFloat", "Twist", "Design", "Rotate around Axis in degrees"
        ).Twist = Twist
    ...
    def createLeadingEdge(self, obj):
        ...
        leadingEdge = FreeCAD.activeDocument().addObject(
            "Sketcher::SketchObject", "LeadingEdge"
        )

        leadingEdge.Placement(
            FreeCAD.Vector(0.000000, 0.000000, 0.000000),
            FreeCAD.Rotation(0.000000, 0.000000, 0.000000, 1.000000),
        )
        leadingEdge.addGeometry(
            Part.LineSegment(
                FreeCAD.Vector(-410.985718, 84.541397, 0),
                FreeCAD.Vector(-375.014648, 308.281403, 0),
            ),
            False,
        )
    ...
    def createRib(self, obj, chord, posvec, direction, rotaxis, angle):
        ...
        if obj.Base.NacaProfil == "":
            WingRib(
                ...
                rotaxis.x,
                rotaxis.y,
                rotaxis.z,  # obj.Base.Placement.Rotation.Axis.x,obj.Base.Placement.Rotation.Axis.y, obj.Base.Placement.Rotation.Axis.z,
    ...
    def updateRib(self, obj, chord, posvec, direction, rotaxis, angle, item):
        ...
        bbox = CurvedShapes.boundbox_from_intersect(
            obj.Hullcurves, posvec, obj.Axis, self.doScaleXYZ
        )
        if not bbox:
            return None
    ...
    def makeRibs(self, obj, update):
        ...
        if update == True:
            ...
            FreeCAD.Console.PrintMessage(
                "  Number of Hullcurves  : " + str(len(obj.Hullcurves)) + "\n"
            )

            # --------------CurvesPathArray code--------------------------
            curvebox = FreeCAD.BoundBox(
                float("-inf"),
                float("-inf"),
                float("-inf"),
                float("inf"),
            ...
            for n in range(0, len(obj.Hullcurves)):
                ...
                if self.doScaleXYZ[n][0]:
                    if cbbx.XMin > curvebox.XMin:
                        curvebox.XMin = cbbx.XMin
                    if cbbx.XMax < curvebox.XMax:
                        curvebox.XMax = cbbx.XMax
                if self.doScaleXYZ[n][1]:
                    if cbbx.YMin > curvebox.YMin:
                        curvebox.YMin = cbbx.YMin
                    if cbbx.YMax < curvebox.YMax:
                        curvebox.YMax = cbbx.YMax
                if self.doScaleXYZ[n][2]:
                    if cbbx.ZMin > curvebox.ZMin:
                        curvebox.ZMin = cbbx.ZMin
                    if cbbx.ZMax < curvebox.ZMax:
                        curvebox.ZMax = cbbx.ZMax
            ...
            for n in range(0, int(obj.Items)):
                ...
                if obj.Items > 1:
                    plen += (
                        (maxlen - obj.OffsetStart - obj.OffsetEnd)
                        * n
                        / (float(obj.Items) - 1)
                    )
                ...
                for edge in edges:
                    ...
                    if plen > edge.Length:
                        ...
                        directionleadingedge = leadingedge_edges[0].tangentAt(
                            param
                        )  # leadinedge direction
                        param_leadingedge = leadingedge_edges[0].getParameterByLength(
                            plen
                        )
                        ...
                        planEdge = Part.makePlane(
                            edge.Length, edge.Length, FreeCAD.Vector(0, 0, 0), direction
                        ).Faces[0]
                        FreeCAD.Console.PrintMessage(
                        ...
                        point = FreeCAD.Vector(
                            posvec_leadingedge.x, posvec_leadingedge.y, posvec_path.z
                        ...
                        if len(obj.Ribs) < obj.Items:
                            ...
                            if bbox:
                                ...
                                chord = scalex
                                rib = self.createRib(
                                    obj, chord, posvecrib, direction, rotaxis, angle
                                )  # posvec_leadingedge
                                FreeCAD.Console.PrintMessage(
                                    "      Rib.Label         : " + str(rib.Label) + "\n"
                                )
                                if rib:
                                    ribs.append(rib)
                            else:
        ...
        if len(ribs) > 1:
            FreeCAD.Console.PrintMessage(
                "     Number of rib created : " + str(len(ribs)) + "\n"
            )
            self.wingpshape(obj)
            if update == False:
                obj.Ribs = ribs
    ...
    def execute(self, prop, update=True):
        ...
        if (
            prop.Items > 0 and len(prop.Hullcurves) > 0
        ):  # and prop.RootRib and hasattr(prop.RootRib, "Shape")
...
class ViewProviderPanel:
    ...
    def claimChildren(self):
        return (
            [self.Object.Base]
            + [self.Object.Path]
            + [self.Object.LeadingEdge]
            + [self.Object.TrailingEdge]
            + self.Object.Ribs
        )  # +[self.Object.TipRib] + [self.Object.RootRib]
    ...
    def GetResources(self):
        ...
        return {
            ...
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "Create_a_wing_Panel", "Create/Add a wing panel, select a panl and clic"
            ),
        }
    ...
    def Activated(self):
        ...
        if selection:
            if len(selection) == 4:
                ...
                if len(edges) != 2:
                    FreeCAD.Console.PrintMessage(
                        "Edges must be 2 and not " + str(len(edges)) + "\n"
                    )
                    return
                ...
                FreeCAD.Console.PrintMessage(
                    "  envelope :" + str(_envelope.Label) + "\n"
                )
                FreeCAD.Console.PrintMessage(
                    "  envelope placement:" + str(_envelope.Placement) + "\n"
                )
                FreeCAD.Console.PrintMessage(
                    "  Number of Panels :" + str(nbOfPanels) + "\n"
                )
                ...
                for i in range(0, nbOfPanels):  # for each panel
                    ...
                    FreeCAD.Console.PrintMessage(
                        "  leadInEdges id        :" + str(leadInEdges[i]) + "\n"
                    )
                    FreeCAD.Console.PrintMessage(
                        "  leadInEdges Length    :" + str(leadInEdges[i].Length) + "\n"
                    )
                    FreeCAD.Console.PrintMessage(
                        "  trailingEdges id      :" + str(trailingEdges[i]) + "\n"
                    )
                    FreeCAD.Console.PrintMessage(
                        "  trailingEdges Length  :"
                    ...
                    v1 = FreeCAD.Vector(
                        0, 1, 0
                    ).normalize()  # paths[i].tangentAt(param).normalize()
                    ...
                    FreeCADGui.activeDocument().activeView().setCameraOrientation(
                        _path.Placement.Rotation
                    )

                    # paths[i].

                    FreeCAD.Console.PrintMessage(
                        "     pathline   : " + str(paths[i]) + "\n"
                    )
