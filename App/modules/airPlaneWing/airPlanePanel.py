...
try:
    ...
    from freecad_imports import App, Gui, Vector, Console, FreeCAD, FreeCADGui
...
class WPanel:
    def __init__(self, obj, _NberOfPanel, _panelInput):
        ...
        obj.Proxy = self
        obj.addProperty(
            "App::PropertyInteger", "NberOfPanel", "WingPanel", "Number of Panel"
        ).NberOfPanel = _NberOfPanel
        ...
        for i in range(0, obj.NberOfPanel):
            ...
            _ribs.append(
                FreeCAD.ActiveDocument.addObject(
                    "Part::FeaturePython", "RibRoot_" + str(i)
                )
            )
            ...
            _ribs.append(
                FreeCAD.ActiveDocument.addObject(
                    "Part::FeaturePython", "RibTip_" + str(i)
                )
            )
    ...
    def Activated(self):
        ...
        if r:
            for row_number in range(
                editor.form.PanelTable.rowCount()
            ):  # -1):#int(editor.form.PanelTable.rowCount())):
                rowData = []
                for col_number in range(
                    10
                ):  # int(editor.form.PanelTable.columnCount())):
                    rowData.append(
                        editor.form.PanelTable.item(row_number, col_number).text()
                    )
                PanelTable.append(rowData)

            a = FreeCAD.ActiveDocument.addObject(
                "App::DocumentObjectGroupPython", "Wing"
            )  # Path::FeaturePython","wpanel") #"Part::FeaturePython","wpanel")
